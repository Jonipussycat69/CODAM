...
/*   Updated: 2023/11/16 18:06:12 by jdobos           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_arg_void(void *arg, t_va *v, t_fl *f)
{
	...
	if (v->spec == 2 && !arg && f->p_len > 0)
		...
		v->print = add_p_f(v->print, ptr_str(arg, v->print));
	if (!(v->print))
		return (NULL);
	return (v->print);
}
...
char	*get_arg_u(unsigned int arg, t_va *v, t_fl *f)
{
	if (v->spec == 6 && f->p_len > 0)
		...
		v->print = add_p_f(v->print, mod_itoa(arg, 10, v->print, 0));
	if (!(v->print))
		return (NULL);
	return (v->print);
}
...
char	*get_arg_int(int arg, t_va *v, t_fl *f)
{
	if (v->spec == 1 && f->p_len > 0)
		...
		v->print = add_p_f(v->print, mod_itoa(arg, 16, v->print, 0));
	else if (v->spec == 8)
		v->print = add_p_f(v->print, mod_itoa(arg, 16, v->print, 1));
	if (!(v->print))
		return (NULL);
	return (v->print);
}
